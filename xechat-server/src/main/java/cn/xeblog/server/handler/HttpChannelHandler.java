...
public class HttpChannelHandler extends AbstractDefaultChannelHandler<FullHttpRequest> {
    ...
    private void handlePushSubscribe(FullHttpRequest request, FullHttpResponse response) {
        ...
        try {
            ...
            PushSubscriptionService.upsert(user.getAccountId(), endpoint, p256dh, auth, json.getStr("pathPrefix"));
    ...
    private void handleDuoAttachmentDownload(FullHttpRequest request, String path, FullHttpResponse response) {
        String prefix = "/api/duo-spaces/";
        String rest = path.substring(prefix.length());
        String[] parts = rest.split("/attachments/");
        if (parts.length != 2) {
        ...
        try {
            byte[] bytes = DuoAttachmentService.read(user.getAccountId(), parts[0], parts[1]);
            response.content().writeBytes(bytes);
        ...
        } catch (DuoAttachmentService.NotFoundException e) {
            response.setStatus(HttpResponseStatus.NOT_FOUND);
        } catch (IllegalArgumentException e) {
